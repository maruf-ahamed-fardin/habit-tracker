...
import { easeOut, itemVariants, springSnappy } from '@/lib/motion'
...
export function HabitCard({
  ...
  // Where the last tap landed, so the colour blooms from that point.
  const [origin, setOrigin] = useState({ x: '85%', y: '50%' })
  const [pulse, setPulse] = useState(0)

  const remember = (e: React.PointerEvent) => {
  ...
  return (
    <motion.button
      ...
      style={{ '--c': color, '--x': origin.x, '--y': origin.y } as React.CSSProperties}
      className={cn(
        ...
        'transition-[border-color,box-shadow] duration-300 hover:shadow-[0_6px_20px_-10px_rgb(0_0_0/0.2)] focus-visible:ring-3 focus-visible:ring-ring/50',
    ...
    >
      ...
      <motion.span
        aria-hidden="true"
        animate={checked ? { rotate: [0, -8, 6, 0], scale: [1, 1.12, 1] } : { rotate: 0, scale: 1 }}
        ...
        <span className="flex flex-wrap items-center gap-x-2.5 gap-y-1 text-xs text-muted-foreground">
          <span className="flex gap-[3px]" aria-hidden="true">
            {week.map(d => (
              <motion.i
                key={d.date}
                animate={d.done ? { scale: [1, 1.6, 1] } : { scale: 1 }}
                transition={{ duration: 0.45, ease: easeOut }}
                className={cn(
                  'block size-[7px] rounded-full bg-border transition-colors duration-300',
                  d.done && 'bg-[var(--c)]',
          ...
          <span className={cn('tabular-nums transition-colors', goalMet && 'font-medium text-foreground')}>
      ...
      <span className="relative grid size-8 shrink-0 place-items-center">
        {pulse > 0 && checked && (
          <span key={pulse} aria-hidden="true" className="check-pulse absolute inset-0 rounded-full bg-[var(--c)]" />
        )}
        <motion.span
          aria-hidden="true"
          animate={checked ? { scale: [1, 1.28, 1] } : { scale: 1 }}
          transition={{ duration: 0.5, ease: [0.34, 1.56, 0.64, 1] }}
          className={cn(
            'relative grid size-8 place-items-center rounded-full border-2 border-border transition-colors duration-300',
